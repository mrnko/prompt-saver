...
- проєкти: призначення промпту, окремий перегляд активних і виконаних тасків та лічильники по кожному проєкту;
...
Завантажте новий installer, закрийте Prompt Saver через меню в треї та запустіть installer поверх наявної версії. Не видаляйте застосунок і не очищуйте `%LOCALAPPDATA%/ua.promptsaver.desktop`: інсталятор оновить файли програми, а база даних і ключ OpenAI залишаться на місці. При запуску застосунок автоматично застосовує безпечні міграції бази; усі попередні промпти лишаються у «Загальному списку».
