...
import { formatDate, promptComplexity, promptPreview, promptTitle } from '../lib/prompt';
...
export function PromptCard({ prompt, onToggle, onEdit, onDelete, onCopy }: Props) {
  ...
  const complexity = promptComplexity(prompt.text);
  return <article className={`prompt-card ${completed ? 'completed' : ''}`}>
    ...
    <div className="prompt-copy"><h3>{promptTitle(prompt.text)}</h3><p>{promptPreview(prompt.text)}</p><div className="complexity-bar" title={`${complexity.label}: ${prompt.text.trim().length} символів`} aria-label={complexity.label}><span className={complexity.tone} style={{ width: `${complexity.percent}%` }}/></div><div className="prompt-meta">{prompt.projectName && <span className="project-chip"><Folder size={11}/>{prompt.projectName}</span>}<time>{completed ? `Виконано ${formatDate(prompt.completedAt!)}` : `Створено ${formatDate(prompt.createdAt)}`}</time></div></div>
